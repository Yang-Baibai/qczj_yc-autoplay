...
re_time = re.search('(\w+):(\w+)',str(date))
minute = 60-int(re_time.group(2))
...
# 主函数
def run():
        pyautogui.alert(text='汽车之家自动发文程序即将运行,请确认后台是否登录,地址栏输入法是否为英文.如需使用推荐名额请等程序运行结束后手动修改.', title='汽车之家自动发文', button='OK')
